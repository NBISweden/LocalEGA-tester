...
from .mq_ops import submit_cega, get_corr, check_mq_ssl
...
def fixture_step_completed(config, current_id, output_base):
    ...
    amqps_set_params = partial(check_mq_ssl, config['tls_ca_root_file'],
                               f"tester{config['tls_cert_ending']}",
                               f"tester{config['tls_key_ending']}")
    fileID = 0
    ...
    get_corr(cm_protocol, config['cm_address'], config['cm_user'],
             ...
             amqps_set_params, config['cm_pass'],
             port=config['cm_port'])
...
def dependency_make_cega_stableID(config, fileID, correlation_id, stableID):
    ...
    """
    cm_protocol = 'amqps' if config['cm_ssl'] else 'amqp'
    amqps_set_params = partial(check_mq_ssl, check_mq_ssl, config['tls_ca_root_file'],
                               f"tester{config['tls_cert_ending']}",
                               f"tester{config['tls_key_ending']}")
    submit_cega(cm_protocol, config['cm_address'], config['cm_user'], config['cm_vhost'],
                {'file_id': fileID, 'stable_id': stableID}, 'stableIDs',
                config['cm_pass'],
                amqps_set_params, correlation_id, port=config['cm_port'])
